...
-- This file continues the migration for the community module
-- Date: 2025-06-07
...
USING (
  event_id IN (
    ...
    (
      ...
      EXISTS (SELECT 1 FROM community_groups WHERE id = ANY(target_groups) AND (access_level::text = 'public' OR id IN (SELECT group_id FROM group_memberships WHERE user_id = auth.uid() AND status = 'active')))
    )
  )
);
