...
const GoogleCalendarPanel: React.FC<GoogleCalendarPanelProps> = ({ expertId, onDisconnect }) => {
  ...
  const checkConnectionStatus = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('google_calendar_credentials, integration_type')
        .eq('user_id', expertId)
        .single();
      ...
      const isGoogleConnected = 
        data?.integration_type === 'google_calendar' && 
        data?.google_calendar_credentials !== null;
      
      setIsConnected(isGoogleConnected);
      
      if (isGoogleConnected && data?.google_calendar_credentials) {
        // In a real implementation, we would decrypt the credentials
        // and extract the email from the token payload
        // For now, we'll use a placeholder
        setCalendarEmail(data.google_calendar_credentials.email || 'your.email@gmail.com');
      } else {
  ...
  const handleConnect = async () => {
    try {
      ...
      // Mock credentials that would be returned from Google
      ...
      const { error } = await supabase.rpc('connect_google_calendar', {
      ...
      setIsConnected(true);
      setCalendarEmail('your.email@gmail.com');
      toast.success('Google Calendar Connected', 'Your calendar has been successfully connected.');
      
      // Refresh the connection status
      checkConnectionStatus();
    } catch (error) {
      console.error('Error in handleConnect:', error);
      toast.error('Connection Error', 'Failed to connect Google Calendar. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDisconnect = async () => {
    try {
      ...
      // Call the disconnect_google_calendar function
      ...
      if (error) {
        ...
        toast.error('Failed to disconnect Google Calendar', 'Please try again later.');
        return;
      }
      
      setIsConnected(false);
      setCalendarEmail(null);
      toast.success('Google Calendar Disconnected', 'Your calendar has been successfully disconnected.');
      ...
      toast.error('Disconnection Error', 'Failed to disconnect Google Calendar. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white shadow sm:rounded-lg p-6">
        ...
        <div className="prose prose-sm text-gray-500 mb-6">
          <p>
            ...
            When connected, we'll check your Google Calendar for conflicts when clients
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 text-sm">
                ...
                <p className="text-gray-500">
                  Check availability across all your Google Calendars
                </p>
              </div>
            </div>
          </div>
