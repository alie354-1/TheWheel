...
CREATE INDEX IF NOT EXISTS idx_discussion_threads_group_id ON discussion_threads(group_id);
...
CREATE INDEX IF NOT EXISTS idx_expert_endorsements_expert_id ON expert_endorsements(expert_id);
CREATE INDEX IF NOT EXISTS idx_achievements_user_id ON achievements(user_id);
CREATE INDEX IF NOT EXISTS idx_contribution_scores_user_id ON contribution_scores(user_id);
CREATE INDEX IF NOT EXISTS idx_contribution_scores_period ON contribution_scores(scoring_period, period_start);
...
USING (
  ...
  (
    ...
    EXISTS (SELECT 1 FROM community_groups WHERE id = ANY(target_groups) AND (community_groups.access_level)::text = 'public')
  )
);
