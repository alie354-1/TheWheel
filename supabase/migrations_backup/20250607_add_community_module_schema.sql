...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_community_events_start_date') THEN
...
USING (
  target_groups IS NULL OR
  array_length(target_groups, 1) IS NULL OR
  EXISTS (SELECT 1 FROM community_groups WHERE id = ANY(target_groups) AND id IN (SELECT group_id FROM group_memb
