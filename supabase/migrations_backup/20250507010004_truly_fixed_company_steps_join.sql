-- Fix the column references in Business Operations Hub views and add necessary columns
-- Created: May 10, 2025 - TRULY FIXED VERSION with CASCADE for view dependencies

-- First, add the company_id column to company_journey_steps if it doesn't exist
ALTER TABLE company_journey_steps 
ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES companies(id);

-- Update existing company_journey_steps to set their company_id if possible
-- This uses the first company available in the database as a temporary measure
UPDATE company_journey_steps cjs
SET company_id = (SELECT id FROM companies LIMIT 1)
WHERE company_id IS NULL;

-- Create an index on the company_id column for better performance
CREATE INDEX IF NOT EXISTS idx_company_journey_steps_company_id ON company_journey_steps(company_id);

-- Drop existing views that use the incorrect join - use CASCADE to drop dependencies
DROP VIEW IF EXISTS domain_step_statistics CASCADE;
DROP VIEW IF EXISTS domain_steps_status CASCADE;
DROP VIEW IF EXISTS domain_statistics CASCADE;

-- Recreate domain_statistics view with the correct column names
...
LEFT JOIN
  company_journey_steps cjs ON js.id = cjs.global_step_id AND cjs.company_id IS NOT NULL
GROUP BY 
  bd.id, bd.name;

-- Recreate domain_steps_status view with the correct join column
...
-- Recreate domain_step_statistics view with correct references
