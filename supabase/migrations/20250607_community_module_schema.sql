...
USING (access_level::text = 'public' AND NOT is_archived);
...
ON community_groups FOR SELECT 
USING (access_level::text IN ('public', 'extended_ecosystem') AND NOT is_archived);
...
DROP POLICY IF EXISTS "Users can create reactions to content they can see" ON content_reactions;
...
DROP POLICY IF EXISTS "Users can delete their own reactions" ON content_reactions;
...
USING (
  ...
  (
    ...
    EXISTS (SELECT 1 FROM community_groups WHERE id = ANY(target_groups) AND access_level::text = 'public')
  )
);

DROP POLICY IF EXISTS "Users can view events for their groups" ON community_events;
